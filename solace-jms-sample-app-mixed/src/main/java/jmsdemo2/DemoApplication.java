...
@SpringBootApplication public class DemoApplication {
  ...
  @Service static class MessageProducer implements CommandLineRunner {

    private static final Logger logger = LoggerFactory.getLogger(MessageProducer.class);

    @Autowired() @Qualifier("jndiJmsTemplate") private JmsTemplate jmsTemplate;

    @Autowired() @Qualifier("jmsPublisher") private JmsTemplate publisher;

    @Autowired() @Qualifier("jndiJmsPublisher") private JmsTemplate jndiPublisher;
    ...
    @Value("${solace.jms.demoConsumerQueueJndiName}") private String queueName;

    @Value("${solace.jms.demoTopicName}") private String topicName;

    @Value("${solace.jms.demoTopicName2}") private String topicName2;

    @Override public void run(String... strings) throws Exception {
      String msg = "Hello World";
      MessagePostProcessor postProcessor = new MessagePostProcessor() {
        @Override public javax.jms.Message postProcessMessage(javax.jms.Message message)
          throws JMSException {
          message.setBooleanProperty("JMS_Solace_isXML", false);
          message.setIntProperty("Solace_JMS_CompressionLevel", 1);
          return message;
        }
      };
      logger.info("============= Sending to topic " + topicName + ", msg=" + msg);
      this.publisher.convertAndSend(topicName, msg, postProcessor);

      logger.info("============= Sending to queue " + queueName + ", msg=" + msg);
      try {
        this.jmsTemplate.convertAndSend(queueName, msg, postProcessor);
      } catch (Exception e) {
      ...
      try {
        this.publisher.convertAndSend(topicName2, msg, postProcessor);
      } catch (Exception e) {
  ...
  @Component static class MessageHandler {
  ...
  @Component static class TopicMessageHandler {
