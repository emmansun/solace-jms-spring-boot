...
@Configuration public class ProducerConfiguration {
  ...
  @Bean public CachingConnectionFactory cachingConnectionFactory() {
    CachingConnectionFactory ccf =
      new CachingConnectionFactory((ConnectionFactory) connectionFactory().getObject());
  ...
  @Bean public JmsTemplate jmsPublisher() {
  ...
  @Bean public JmsTemplate jndiJmsTemplate() {
    JmsTemplate jmsTemplate = new JmsTemplate(cachingConnectionFactory());
    jmsTemplate.setDestinationResolver(jndiDestinationResolver());
    return jmsTemplate;
  }

  @Bean public JmsTemplate jndiJmsPublisher() {
    JmsTemplate jmsTemplate = new JmsTemplate(cachingConnectionFactory());
    jmsTemplate.setDestinationResolver(jndiDestinationResolver());
    jmsTemplate.setPubSubDomain(true);
    return jmsTemplate;
